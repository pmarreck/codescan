...
- [x] Repair the Windows linker assumptions and reverify the final ReleaseFast five-target fleet. (completed 2026-08-06 11:15 EDT)
  The stale claim that Windows passed masked two final-link failures: MinGW redirects `localtime_s` to `_localtime64_s`, and Windows has no `syslog` ABI. Unix system logging remains on macOS/Linux; the project-local watcher failure record remains the durable Windows diagnostic. Fresh ReleaseFast builds emitted the primary `codescan` executable for macOS ARM64, Linux ARM64/x86_64, and Windows ARM64/x86_64.
  Curiosity poke: a compile that emits helper tools but fails its primary executable is not a successful target; inspect the requested binary for every fleet member.
- [x] Diagnose why `codescan watcher start` immediately exits for `~/Code/random`. (completed 2026-08-06 11:15 EDT)
  Foreground and independently detached runs against the reported project stayed alive, and the detached watcher retained its PID after its parent terminal ended. The observed immediate stopped state was a launcher/early-status race in the agent command host, not a watcher self-retirement, inference preflight failure, or project configuration condition. The daemon now leaves evidence that distinguishes future genuine failures.
  Curiosity poke: distinguish the parent falsely reporting a dead child from intentional self-retirement, provider-preflight failure, or a project configuration mismatch.
- [x] Make watcher startup and runtime failures visible to the invoking CLI or a durable project/XDG log. (completed 2026-08-06 11:15 EDT)
  Pre-fork errors remain on the CLI. Daemon-only returns now save the bounded latest reason in `.codescan/watcher-error.log` and mirror it to the Unix OS log; `watch status` reports that path when stopped. Native/polling startup records include PID, and the failure-record replacement and daemon-return paths have deterministic tests. Linux acceptance observed `watcher started (native events, PID 1037853)` in journald.
  Curiosity poke: do not make a failed child look like a successful claimed PID; the failure record needs an unambiguous project owner, bounded size, and a clear replacement policy.
- [x] Repair `codescan log --since 2h` duration parsing. (completed 2026-08-06 11:15 EDT)
  Linux now maps documented positive shorthand (`30s`, `15m`, `2h`, `1d`) to journald's accepted relative-time grammar and rejects malformed, zero, and overflowing values with a specific CLI/MCP message. The error exits 64 and uses stderr, so stdout remains log data. The actual `2h` command returned watcher logs, and unit plus black-box CLI cases cover the parser and stream split. Filed as GitHub issue #1 before the repair.
  Curiosity poke: keep units and boundary values explicit so `2h`, `15m`, bad suffixes, zero, and overflow do not collapse into the same result.
- [x] Make live HTTP/integration test preflight bounded and skip cleanly when Ollama is intentionally unavailable. (completed 2026-08-06 11:25 EDT)
  `./test` reached the live integration probe after Peter stopped Ollama, then its unbounded `curl` capture kept the runner occupied rather than reporting a controlled environmental skip. The preflight now uses a bounded probe, skips live suites by default when its server/model is absent, and retains `CODESCAN_REQUIRE_LIVE_OLLAMA=1` for a strict run. Closed-port HTTP/integration acceptance both skipped cleanly; the HTTP live suite also passed against the restored server; `OLLAMA_URL=http://127.0.0.1:1 ./test` passed all layers.
  Curiosity poke: a missing external service must neither make a deterministic suite red nor mask a failed live run the caller explicitly requested.
...
- [~] Bound startup preflight metadata calls at 10s, warning to stderr above 3s. **Implemented and proved on Linux; native macOS/Windows acceptance remains.**
  ...
  - [x] Repair the Windows local-time and system-log link assumptions. (completed 2026-08-06 11:10 EDT)
    MinGW's public `localtime_s` declaration has an assembler alias that Zig's C import does not preserve, so `writeLocalTime` calls the exported `_localtime64_s` entry point on Windows. `syslog.zig` compiles Unix log calls out on Windows; watcher daemon failures already have a project-local `.codescan/watcher-error.log` record there. Both Windows executable targets now emit `codescan.exe`; the all-five final verification above passed.
