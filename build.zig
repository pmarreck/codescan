...
pub fn build(b: *std.Build) void {
    ...
    const ts_clojure = buildTreeSitterGrammar(b, target, optimize, "tree_sitter_clojure", "deps/tree-sitter-clojure/src", false);
    const ts_asm = buildTreeSitterGrammar(b, target, optimize, "tree_sitter_asm", "deps/tree-sitter-asm/src", false);
    const grammar_root = b.graph.environ_map.get("CODESCAN_GRAMMAR_ROOT") orelse
        @panic("CODESCAN_GRAMMAR_ROOT is unset; build codescan through its Nix flake");
    const ts_fish = buildTreeSitterGrammar(b, target, optimize, "tree_sitter_fish", b.fmt("{s}/fish", .{grammar_root}), true);
    const ts_nu = buildTreeSitterGrammar(b, target, optimize, "tree_sitter_nu", b.fmt("{s}/nu", .{grammar_root}), true);
    const ts_powershell = buildTreeSitterGrammar(b, target, optimize, "tree_sitter_powershell", b.fmt("{s}/powershell", .{grammar_root}), true);
    const ts_tcl = buildTreeSitterGrammar(b, target, optimize, "tree_sitter_tcl", b.fmt("{s}/tcl", .{grammar_root}), true);
    const ts_fsharp = buildTreeSitterGrammar(b, target, optimize, "tree_sitter_fsharp", b.fmt("{s}/fsharp/fsharp/src", .{grammar_root}), true);
    const ts_elm = buildTreeSitterGrammar(b, target, optimize, "tree_sitter_elm", b.fmt("{s}/elm", .{grammar_root}), true);
    const ts_gleam = buildTreeSitterGrammar(b, target, optimize, "tree_sitter_gleam", b.fmt("{s}/gleam", .{grammar_root}), true);
    const ts_scheme = buildTreeSitterGrammar(b, target, optimize, "tree_sitter_scheme", b.fmt("{s}/scheme", .{grammar_root}), false);
    const ts_commonlisp = buildTreeSitterGrammar(b, target, optimize, "tree_sitter_commonlisp", b.fmt("{s}/commonlisp", .{grammar_root}), false);
    const ts_sml = buildTreeSitterGrammar(b, target, optimize, "tree_sitter_sml", b.fmt("{s}/sml", .{grammar_root}), true);
    const ts_wat = buildTreeSitterGrammar(b, target, optimize, "tree_sitter_wat", b.fmt("{s}/wat", .{grammar_root}), true);
...
fn buildTreeSitterGrammar(
    ...
    module.addCSourceFile(.{
        .file = sourcePath(b, b.fmt("{s}/parser.c", .{dir})),
    });
    if (has_scanner) {
        module.addCSourceFile(.{
            .file = sourcePath(b, b.fmt("{s}/scanner.c", .{dir})),
        });
    }
    module.addIncludePath(sourcePath(b, dir));
    module.addIncludePath(b.path("deps/tree-sitter/lib/include"));
    module.addIncludePath(b.path("deps/tree-sitter/lib/src"));
    return lib;
}

fn sourcePath(b: *std.Build, path: []const u8) std.Build.LazyPath {
