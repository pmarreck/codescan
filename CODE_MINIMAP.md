...
- LICENSE: MIT license
- tests/integration/test-integration: end-to-end Ollama suite with model/dimension pairing and clones/indexes isolated under RAM-backed TMPDIR
...
- src/storage.zig: sqlite + sqlite-vec schema init (static vec init, optional FTS5), schema migrations, embedding model/dim mismatch detection, crash-recovery completeness checks via linear rowid-set scans, index reset, inserts, tests
