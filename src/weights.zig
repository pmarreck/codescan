...
pub fn resolveSearchWeights(
	...
	if (table) |value| {
		return value.resolveForLanguages(allowed_langs, base_vector, base_lexical);
	}
	return .{
		.weight_vector = base_vector,
		.weight_lexical = base_lexical,
...
pub fn parseText(allocator: std.mem.Allocator, text: []const u8) !Table {
	...
	while (lines.next()) |line| {
		...
		if (!std.mem.eql(u8, key, "weight_vector") and !std.mem.eql(u8, key, "weight_lexical")) {
...
test "parseText reads default and per-language overrides" {
	...
	const zig_pair = table.resolveForLanguages(&[_][]const u8{"zig"}, 0.7, 0.3);
	...
	const ex_pair = table.resolveForLanguages(&[_][]const u8{"elixir"}, 0.7, 0.3);
...
test "resolveForLanguages keeps defaults when multiple languages are active" {
	...
	const pair = table.resolveForLanguages(&[_][]const u8{ "zig", "markdown" }, 0.7, 0.3);
