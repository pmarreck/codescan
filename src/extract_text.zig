...
test "extract returns no symbols on empty source" {
	const allocator = std.testing.allocator;
	const symbols = try extract(allocator, "src/empty.txt", "");
...
test "extract emits at least one symbol for a non-empty source" {
	const allocator = std.testing.allocator;
	const source = "One sentence here.\n";

	const symbols = try extract(allocator, "src/one.txt", source);
	defer {
		for (symbols) |*sym| sym.deinit(allocator);
		allocator.free(symbols);
	}
...
test "extract handles UTF-8 content without crashing" {
	...
	const symbols = try extract(allocator, "src/utf8.txt", source);
	defer {
		for (symbols) |*sym| sym.deinit(allocator);
		allocator.free(symbols);
	}
