...
pub fn extract(
	...
	if (!ts.ts_parser_set_language(parser, tree_sitter_nim())) {
...
fn isCallableDecl(node: ts.TSNode) bool {
	const ty = std.mem.span(ts.ts_node_type(node));
	return std.mem.eql(u8, ty, "proc_declaration") or
		std.mem.eql(u8, ty, "func_declaration") or
		std.mem.eql(u8, ty, "method_declaration") or
...
fn extractFunction(
	...
	const symbol = model.Symbol{
		.language = try allocator.dupe(u8, "nim"),
...
test "extract finds nim procs" {
	const allocator = std.testing.allocator;
	const source =
		"## adds\n" ++
		"proc add(a: int, b: int): int = a + b\n";

	const symbols = try extract(allocator, "src/lib.nim", source);
	defer {
...
test "extract returns no symbols on empty source" {
	const allocator = std.testing.allocator;
	const symbols = try extract(allocator, "src/empty.nim", "");
...
test "extract handles declaration with no preceding comment (doc_comment null)" {
	const allocator = std.testing.allocator;
	const source = "proc plain(a: int): int = a\n";

	const symbols = try extract(allocator, "src/nodoc.nim", source);
	defer {
		for (symbols) |*sym| sym.deinit(allocator);
		allocator.free(symbols);
	}
...
test "extract attaches preceding comment as doc_comment" {
	...
	const symbols = try extract(allocator, "src/withdoc.nim", source);
	defer {
		for (symbols) |*sym| sym.deinit(allocator);
		allocator.free(symbols);
	}
...
test "extract handles UTF-8 content without crashing" {
	...
	const symbols = try extract(allocator, "src/utf8.nim", source);
	defer {
		for (symbols) |*sym| sym.deinit(allocator);
		allocator.free(symbols);
	}
