# Project Plan

## Objective
Build `codescan`: a Zig CLI for semantic code search using Ollama embeddings (bge-large) and sqlite-vec, with a plugin architecture starting with Zig and Elixir extractors.

## Milestones
- M1: CLI scaffolding + config + tests
- M2: Storage layer + embeddings client
- M3: Indexing pipeline
- M4: Search pipeline + output formatting + hybrid weight knobs
- M5: Zig and Elixir plugins
- M6: Docs + stability pass
